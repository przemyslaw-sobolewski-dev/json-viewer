...
export function getComponentSlices(
  ...
  const addEllipsisToSlices = (
    ...
    const componentSlices = slices.filter(
      (slice): slice is ComponentSlice => slice.type === "component"
    );
  ...
  while (weight > maxWeight) {
    ...
    const sortedByLongestMatch = Object.entries(groupByComponentIndex).sort(
      ([, componentSlicesA], [, componentSlicesB]) => {
        if (
          calculateLongestMatch(componentSlicesA as ComponentSlice[]) >
          calculateLongestMatch(componentSlicesB as ComponentSlice[])
        ) {
          return -1;
        }
